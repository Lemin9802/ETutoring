...
interface ChatBoxProps {
    chatId?: string;      // Dùng để fetch tin nhắn (conversation id)
    chatroomId: string;   // Dùng khi gửi tin nhắn (chatroom id)
    recipientId: string;
...
const ChatBox: React.FC<ChatBoxProps> = ({
    ...
    // SignalR connection (listener) dùng chatroomId không cần thay đổi
    useEffect(() => {
                        ...
                        (senderId: string, receiverId: string, message: string) => {
                            ...
                            if (receiverId === userId) {
                                const newMsg: Message = {
                                    ...
                                    content: message,
                                    timestamp: new Date(),
                                    chatroom_id: chatroomId, // Gán chatroom_id từ prop
                                };
    ...
    // Fetch tin nhắn ban đầu: sử dụng chatId (conversation id) để fetch
    useEffect(() => {
        const fetchMessages = async () => {
            ...
            try {
                const {data} = await axios.post("/api/messages/get-message", {
                    ...
                    chatroom_id: chatId, // Sử dụng chatId khi fetch tin nhắn
                });
    ...
    // Hàm gửi tin nhắn: sử dụng chatroomId để gửi payload
    const handleSendMessage = async () => {
        ...
        const tempMessage: Message = {
            ...
            content: newMessage,
            timestamp: new Date(),
            chatroom_id: chatroomId, // Gửi chatroom_id từ prop
        };

        // Optimistic UI: thêm tin nhắn vào state ngay lập tức
        ...
        try {
            await axios.post("/api/messages/send-message", {
                ...
                chatroom_id: chatroomId, // Sử dụng chatroomId khi gửi tin nhắn
            });
