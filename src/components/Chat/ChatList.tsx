...
interface ChatListProps {
  onSelectChat: (chat: { chatId: string; recipientId: string; recipientName: string; recipientAvatar?: string }) => void;
      ...
      {chats.length === 0 && !loading ? (
        ...
        <List
          ...
          renderItem={(chat) => {
            ...
            return (
              <List.Item
                className={`cursor-pointer hover:bg-gray-50 ${selectedChatId === chat.conversation_id ? "bg-gray-100" : ""}`}
                onClick={() =>
                  onSelectChat({
