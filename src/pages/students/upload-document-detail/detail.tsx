...
import { PlusOutlined, MinusOutlined, DownloadOutlined, InfoCircleOutlined, CloseOutlined } from '@ant-design/icons';
import Feedback from "@/pages/students/upload-document-detail/feedback";
...
const PdfViewer: React.FC = () => {
    const pdfUrl: string = 'https://etutoring-storage.s3.amazonaws.com/documents/4e5fd223-89fd-47bd-a1ae-86fd04827e20-Git_Commit_Message_Standard.pdf';
    ...
    const handleDownload = (): void => {
        fetch(fileUrl)
            ...
            .then((blob) => {
                ...
                link.download = 'Git_Commit_Message_Standard.pdf';
                link.click();
    ...
    return (
        <div style={{ textAlign: 'center', padding: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', position: 'absolute', right: '20px', top: '25%', transform: 'translateY(-50%)' }}>
                    ...
                    <div style={modalOverlayStyle}>
                        <div style={infoStyle}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <h2 style={infoHeaderStyle}><strong>Information</strong></h2>
                                <button onClick={handleCloseModal} style={closeButtonStyle}>
                                    <CloseOutlined />
                                </button>
                            </div>
                            <p><strong>Name:</strong> Git_Commit_Message_Standard</p>
                            <p><strong>Size:</strong> {fileSize ?? 'Loading...'}</p>
                            <p><strong>Pages:</strong> {numPages ?? 'Loading...'}</p>
                            <p><strong>URL:</strong> <a href={pdfUrl} target="_blank" rel="noopener noreferrer">{pdfUrl}</a></p>
                        </div>
                    </div>
                )}
            </div>
