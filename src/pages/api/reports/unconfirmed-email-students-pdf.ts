...
export default async function handler(
  ...
  try {
    const backendResponse = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/reports/students/unconfirmed-email/pdf`, // Updated endpoint
      {
        method: "POST",
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return res.status(backendResponse.status).json({
        error: `Failed to fetch unconfirmed email students PDF report. Status: ${backendResponse.status}`,
      });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=UnconfirmedEmailStudentsReport.pdf" // Updated filename
    );
    ...
    console.error(
      "Failed to fetch unconfirmed email students PDF report",
      error
    );
    return res.status(500).json({
      error:
        "Internal Server Error while fetching unconfirmed email students PDF report.",
    });
  }
}
