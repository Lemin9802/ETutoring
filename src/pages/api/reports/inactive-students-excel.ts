...
export default async function handler(
  ...
  try {
    const backendResponse = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/reports/students/inactive/excel`,
      {
        method: "POST",
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return res.status(backendResponse.status).json({
        error: `Failed to fetch inactive students Excel report. Status: ${backendResponse.status}`,
      });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=InactiveStudentsReport_${days}days.xlsx`
    ...
    console.error("Failed to fetch inactive students Excel report", error);
    return res.status(500).json({
      error:
        "Internal Server Error while fetching inactive students Excel report.",
    });
  }
}
