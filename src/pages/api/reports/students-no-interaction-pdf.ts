  ...
  try {
    ...
    if (!backendResponse.ok) {
      ...
      return res.status(backendResponse.status).json({
        error: `Failed to fetch students without interaction PDF report. Status: ${backendResponse.status}`,
      });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=StudentsWithoutInteractionReport_${days}days.pdf` // Updated filename
    ...
    console.error(
      "Failed to fetch students without interaction PDF report",
      error
    );
    return res.status(500).json({
      error:
        "Internal Server Error while fetching students without interaction PDF report.",
    });
  }
}
