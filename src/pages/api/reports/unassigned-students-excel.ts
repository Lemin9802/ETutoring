...
export default async function handler(
  ...
  try {
    const backendResponse = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/reports/students/unassigned/excel`,
      {
        method: "POST",
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return res.status(backendResponse.status).json({
        error: `Failed to fetch unassigned students Excel report. Status: ${backendResponse.status}`,
      });
    }
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=UnassignedStudentsReport.xlsx"
    );
    ...
    console.error("Failed to fetch unassigned students Excel report", error);
    return res.status(500).json({
      error:
        "Internal Server Error while fetching unassigned students Excel report.",
    });
  }
}
