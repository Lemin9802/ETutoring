...
export default async function handler(
  ...
  try {
    const backendResponse = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/reports/students/no-interaction`,
      {
        method: "POST",
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return res.status(backendResponse.status).json({
        error: `Failed to fetch students without interaction data. Status: ${backendResponse.status}`,
      });
    }
    ...
    console.error("Failed to fetch students without interaction data", error);
    return res.status(500).json({
      error:
        "Internal Server Error while fetching students without interaction data.",
    });
  }
}
